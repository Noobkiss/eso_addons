	...
	if doesMapCacheHandlePinType then
		...
		if Harvest.COMBINED_PINTYPE[pinTypeId] then
			for otherPinTypeId in pairs(Harvest.COMBINED_PINTYPE[pinTypeId]) do
				nodeId = mapCache:GetMergeableNode(otherPinTypeId, worldX, worldY, worldZ)
				if nodeId then
					local del_worldX = mapCache.worldX[nodeId]
					local del_worldY = mapCache.worldY[nodeId]
					local del_worldZ = mapCache.worldZ[nodeId]
					
					CallbackManager:FireCallbacks(Events.NODE_DELETED, mapCache, nodeId)
					mapCache:Delete(nodeId)
					self:Info("node was removed from cache due to combined pin type. nodeId: %d", nodeId)
					-- add delete flag to saved variables so data is removed during next start up
					-- additional serialized data
					local serializedNode = Serialization:Serialize(del_worldX, del_worldY, del_worldZ, timestamp, version, Harvest.deleteFlag)
					
					-- always store node
					-- it will be merged on next startup
					savedVars[zoneId][map][otherPinTypeId] = savedVars[zoneId][map][otherPinTypeId] or {}
					local nodeIndex = (#savedVars[zoneId][map][otherPinTypeId]) + 1
					savedVars[zoneId][map][otherPinTypeId][nodeIndex] = serializedNode
