...
function Serialization:LoadNodesOfPinTypeToCache(pinTypeId, mapCache)
	...
	if map:match("/u%d+_") then
		self:Debug("this is a version specific map. it will be loaded to the /uXX_ version of %s", map)
		return
	end
	...
	if map:match("/uXX_") then
		self:Debug("this is a version specific map:", mapCache.map)
		local pattern = "^" .. map:gsub("/uXX_", "/u%%d+_") .. "$"
		if savedVars[zoneId] then
			for map, mapData in pairs(savedVars[zoneId]) do
				if mapData[pinTypeId] and map:match(pattern) then 
					self.serializedNodes = savedVars[zoneId][map][pinTypeId]
					numAddedNodes = numAddedNodes + self:LoadDiscoveredData()
				end
			end
...
function Serialization:LoadDiscoveredData()
	...
	self:Debug("start processing map:", mapCache.map)
		...
		if flags == Harvest.deleteFlag then
			...
			if valid then
				...
				if nodeId then
					...
					self:Debug("node was merged:", node)
