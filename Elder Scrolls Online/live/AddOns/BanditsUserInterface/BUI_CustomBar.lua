...
do	--Default
	local t={Enable=false,Leader={},Slash={}, position={x=0, y=0}}
...
function BUI.CustomBarUpdate(theme,slots,parent)
	...
	-- 修改点：强制使用单行布局
	local half = false  -- 强制设为false，不分行
	local space = 2
	local h = UltimateSlot.slot:GetHeight()  -- 移除half判断，始终使用完整高度
	local height = h  -- 固定为单行高度
	local width = (h+space) * slots  -- 宽度根据按钮数量动态计算
	local anchor = parent and {TOP,parent,BOTTOM,0,10} or {TOPLEFT,parent,TOPRIGHT,20,0}
	local ui = BUI_CustomBar
	parent=parent or ZO_ActionBar1
	-- if ui then ui:SetParent(parent) else ui=WINDOW_MANAGER:CreateControl("BUI_CustomBar", parent, CT_CONTROL) end
	...
	if ui then 
		...
		ui = WINDOW_MANAGER:CreateControl("BUI_CustomBar", parent, CT_CONTROL)
		-- 新增：设置为可移动
		ui:SetMovable(true)
	...
	-- 如果控件已存在但可能没设置 movable，再次确保
	ui:SetMovable(true)
	ui:SetMouseEnabled(true)

	ui:SetDimensions(width,height)

	-- 修改点：只有在没有保存位置时才使用默认锚点
	local useSavedPosition = false

	if BUI.Vars and BUI.Vars.CustomBar and BUI.Vars.CustomBar.position then
		local x, y = BUI.Vars.CustomBar.position.x, BUI.Vars.CustomBar.position.y
		if x and y and (x ~= 0 or y ~= 0) then
			-- 有保存的位置，使用它
			ui:ClearAnchors()
			ui:SetAnchor(TOPLEFT, GuiRoot, TOPLEFT, x, y)
			useSavedPosition = true
		end
	end

	if not useSavedPosition then
		-- 没有保存的位置，使用默认锚点
		ui:ClearAnchors()
		ui:SetAnchor(unpack(anchor))
	end
	...
	if BUI.Vars.CustomBar.Enable then
		for i=1,slots do
			...
			-- 修改点：简化行列计算，所有按钮都在第一行
			local row = 0  -- 固定为第一行
			local col = (i-1)*(h+space)  -- 按钮水平排列
			...
			ui[i].over		=BUI.UI.Texture("BUI_CustomSlot"..i.."DropCallout", ui[i], {h-space,h-space}, {TOPLEFT,TOPLEFT,space/2,space/2}, "/EsoUI/Art/ActionBar/actionBar_mouseOver.dds", true, 2)
			ui[i].text		=BUI.UI.Label("BUI_CustomSlot"..i.."Text", ui[i], {h-space*2,h-space*2}, {TOPLEFT,TOPLEFT,space,space}, BUI.UI.Font("esobold",h*.4,true), nil, {1,1}, BarContent[i].text)
			ui[i].key		=BUI.UI.Label("BUI_CustomSlot"..i.."Key", ui[i], {h,13}, {TOPLEFT,BOTTOMLEFT,0,0}, "ZoFontGameSmall", nil, {1,1}, GetKeyBind(i),false)  -- 最后一个参数改为false
			ui[i].slot=i
			ui[i]:SetMouseEnabled(true)
			ui[i]:SetHandler("OnMouseDown", function(self, button)
				if button == 1 then  -- 左键
			...
			ui[i]:SetHandler("OnMouseUp", function(self, button)
				if button == 2 then  -- 右键释放
					self.dragging = false  -- 清除拖动标记
					local parentCtrl = self:GetParent()
					
					-- 安全判断：确保 parentCtrl 存在且有 StopMovingOrSizing 方法
					if parentCtrl and parentCtrl.StopMovingOrSizing then
						parentCtrl:StopMovingOrSizing()
					end
					
					-- 保存位置
					if not BUI.Vars then BUI.Vars = {} end
					if not BUI.Vars.CustomBar then BUI.Vars.CustomBar = {} end
					if not BUI.Vars.CustomBar.position then 
						BUI.Vars.CustomBar.position = {x=0, y=0}
					end
					
					local left, top = parentCtrl:GetLeft(), parentCtrl:GetTop()
					if left and top then
						BUI.Vars.CustomBar.position.x = left
						BUI.Vars.CustomBar.position.y = top
					end
				end
			end)
			ui[i]:SetHandler("OnMouseEnter", function(self)
			...
			ui[i]:SetHandler("OnMouseExit", function(self)
				self.dragging = false  -- 鼠标离开时也清除标记
				self.over:SetHidden(true)
				ZO_Tooltips_HideTextTooltip()
			end)
		end
