...
function LootLog.InitializeHistory( )
	...
	LEJ.RegisterTab(TAB_NAME, {
		...
		subtitle = SI_LOOTLOG_SUBTITLE_LIST,
		order = 100,
	...
	LCCC.RegisterLinkHandler("lootlog", function() LEJ.Show(TAB_NAME) end)
end
...
function LootLogList:ProcessItemEntry( stringSearch, data, searchTerm, cache )
	if (searchTerm == "~") then
		if (LDRT) then
			return data.time >= LDRT.GetDailyResetTime(true)
		else
			local currentTime = GetGameTimeMilliseconds()
			if (currentTime - (self.ldrtPrev or 0) >= 60000) then
				self.ldrtPrev = currentTime
				LootLog.Msg(string.format("|cFF0000%s:|r %s", GetString(SI_PROMPT_TITLE_ERROR), zo_strformat(SI_ADDON_MANAGER_DEPENDENCY_MISSING, "LibDailyResetTime")))
			end
			return true
		end
 	end
