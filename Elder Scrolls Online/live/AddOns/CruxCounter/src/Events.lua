...
local function onPlayerChanged()
    ...
    -- No Crux in player buffs - FIX: Ensure ring gets updated even when no crux found
    if not foundCrux then
        CC.State:SetStacks(0, false)
        -- FIX: Force update the ring visibility immediately
        if CruxCounter_Display and CruxCounter_Display.ring then
            CruxCounter_Display.ring:UpdateCount(0)
        end
    end
end
