...
function CruxCounter_Ring:Initialize(control)
    ...
    -- FIX: Set initial visibility based on current state and settings
    self:UpdateInitialVisibility()
end
...
function CruxCounter_Ring:SetHideZeroStacks(hideZero)
    CC.Debug:Trace(1, "Ring setHideZeroStacks: <<1>>, current stacks: <<2>>", hideZero, CC.State.stacks)
...
--- FIX: Set initial visibility based on current state
--- @return nil
function CruxCounter_Ring:UpdateInitialVisibility()
    local currentCount = CC.State.stacks or 0
    CC.Debug:Trace(2, "Ring initial visibility - count: <<1>>, hideZeroStacks: <<2>>", currentCount, self.hideZeroStacks)
    
    if self.hideZeroStacks then
        self:SetShowing(currentCount > 0)
    else
        self:SetShowing(true)
    end
end
...
function CruxCounter_Ring:UpdateCount(count)
    CC.Debug:Trace(2, "Ring updating count: <<1>>, hideZeroStacks: <<2>>", count, self.hideZeroStacks)
    
    if self.hideZeroStacks then
        local shouldShow = count > 0
        CC.Debug:Trace(2, "hideZeroStacks enabled - shouldShow: <<1>>", shouldShow)
        self:SetShowing(shouldShow)
    else
        CC.Debug:Trace(2, "hideZeroStacks disabled - always showing")
        self:SetShowing(true)
    end
end