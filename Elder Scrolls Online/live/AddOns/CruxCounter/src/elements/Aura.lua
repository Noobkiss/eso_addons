...
function CruxCounter_Aura:ApplySettings()
    ...
    -- FIX: Update ring visibility with current count after settings are applied
    local currentCount = CC.State.stacks or 0
    CC.Debug:Trace(2, "Aura applying settings - current count: <<1>>", currentCount)
    self.ring:UpdateCount(currentCount)
end
