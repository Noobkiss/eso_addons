...
function AGS:GetFilter(filterId)
    if IsConsoleUI() then
        logger:Warn("GetFilter is currently not implemented on console.")
        return nil
    end
...
function AGS:RegisterFilter(filter)
    if IsConsoleUI() then
        logger:Warn("RegisterFilter is currently not implemented on console.")
        return
    end
...
function AGS:RegisterFilterFragment(filterFragment)
    if IsConsoleUI() then
        logger:Warn("RegisterFilterFragment is currently not implemented on console.")
        return
    end
...
function AGS:RegisterSortOrder(sortOrder)
    if IsConsoleUI() then
        logger:Warn("RegisterSortOrder is currently not implemented on console.")
        return
    end
...
function AGS:GetSellTabBackpackLayouts()
    return AGS.internal.BACKPACK_TRADING_HOUSE_LAYOUT_FRAGMENT_BASIC,
        AGS.internal.BACKPACK_TRADING_HOUSE_LAYOUT_FRAGMENT_ADVANCED
end
